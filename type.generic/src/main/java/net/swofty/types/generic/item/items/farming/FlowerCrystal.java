...
public class FlowerCrystal implements CustomSkyBlockItem, SkullHead, ServerOrb, PlaceEvent {
    ...
    public void onPlace(PlayerBlockPlaceEvent event, SkyBlockPlayer player, SkyBlockItem item) {
        ...
        CrystalDatabase.addCrystal("9265f96f54b78885c46e7d2f86b1c1dbfe643c6060fc7fcc9834c3e3fd595135",
                new Pos(event.getBlockPosition().add(0, 1, 0)), ItemTypeLinker.FLOWER_CRYSTAL);
    }

    @Override
    public Function<ServerCrystalImpl, Block> getOrbSpawnMaterial() {
        return (entity) -> Groups.FLOWERS.get(Math.toIntExact(Math.round(Math.random() * (Groups.FLOWERS.size() - 1)))).block();
    }
