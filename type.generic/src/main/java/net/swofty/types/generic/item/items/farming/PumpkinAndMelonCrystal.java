...
public class PumpkinAndMelonCrystal implements CustomSkyBlockItem, SkullHead, ServerOrb, PlaceEvent {
    @Override
    ...
    public void onPlace(PlayerBlockPlaceEvent event, SkyBlockPlayer player, SkyBlockItem item) {
        ...
        CrystalDatabase.addCrystal("9265f96f54b78885c46e7d2f86b1c1dbfe643c6060fc7fcc9834c3e3fd595135",
                new Pos(event.getBlockPosition().add(0, 1, 0)), ItemTypeLinker.PUMPKIN_AND_MELON_CRYSTAL);
    }

    @Override
    public Function<ServerCrystalImpl, Block> getOrbSpawnMaterial() {
        // 50% chance to spawn a pumpkin, 50% chance to spawn a melon
        if (Math.random() > 0.5) {
            return (entity) -> Material.MELON.block();
        }
        return (entity) -> Material.PUMPKIN.block();
    }

    @Override
    public List<Material> getBlocksToPlaceOn() {
        return List.of(Material.PODZOL, Material.DIRT);
    }
}
