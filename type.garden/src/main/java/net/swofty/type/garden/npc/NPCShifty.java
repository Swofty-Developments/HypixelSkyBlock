package net.swofty.type.garden.npc;

import net.swofty.type.garden.gui.GUIShifty;
import net.swofty.type.generic.event.custom.NPCInteractEvent;
import net.swofty.type.skyblockgeneric.user.SkyBlockPlayer;

public class NPCShifty extends AbstractGardenNpc {
    private static final String TEXTURE = "";
    private static final String SIGNATURE = "";

    public NPCShifty() {
        super("shifty", "Shifty", TEXTURE, SIGNATURE);
    }

    @Override
    public void onClick(NPCInteractEvent event) {
        ((SkyBlockPlayer) event.player()).openView(new GUIShifty());
    }
}
