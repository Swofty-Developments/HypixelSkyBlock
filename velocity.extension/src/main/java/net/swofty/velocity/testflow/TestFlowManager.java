...
public class TestFlowManager {
    ...
    public static void registerTestFlow(String name, String handler, List<String> players, List<ServerConfig> serverConfigs) {
        if (activeTestFlows.containsKey(name)) {
            Logger.warn("Test flow {} is already registered, ignoring duplicate registration", name);
            return;
        }
        ...
        Logger.info("Registered test flow: {} (handler={}, players={}, expectedServers={})",
                name, handler, players, instance.getTotalExpectedServers());
    ...
    public static void markServerReady(String testFlowName, ServerType serverType, int serverIndex, UUID serverUUID) {
        ...
        if (instance == null) {
            Logger.warn("Received server ready notification for unknown test flow: {}", testFlowName);
            return;
        }

        instance.markServerReady(serverType, serverIndex, serverUUID);
        Logger.info("Server {} #{} is ready for test flow: {}", serverType.name(), serverIndex, testFlowName);
        ...
        if (instance.areAllServersReady() && !instance.isAllServersReadyTriggered()) {
            ...
            Logger.info("All servers are ready for test flow: {}", testFlowName);
    ...
    public static void handleServerDisconnect(UUID serverUUID) {
        activeTestFlows.values().forEach(instance -> {
            if (instance.getServerUUIDs().contains(serverUUID)) {
                Logger.info("Test flow server disconnected: {} from test flow: {}", serverUUID, instance.getName());

                instance.removeServer(serverUUID);

                if (instance.hasAnyServersOnline()) {
                    Logger.info("Test flow {} still has {} servers online",
                            instance.getName(), instance.getOnlineServerCount());
                } else {
                    Logger.warn("All servers for test flow {} are offline, cleaning up...", instance.getName());
    ...
    private static void cleanupTestFlow(String testFlowName) {
        ProxyTestFlowInstance instance = activeTestFlows.get(testFlowName);
        if (instance != null) {
            Logger.info("Cleaning up test flow: {}", testFlowName);

            if (instance.getHandler() != null) {
                try {
                    instance.getHandler().onTestFlowEnd(instance);
                } catch (Exception e) {
                    Logger.error(e, "Error in test flow handler onTestFlowEnd");
                }
            }

            activeTestFlows.remove(testFlowName);
            Logger.info("Test flow {} has been cleaned up and removed", testFlowName);
        }
    }
    ...
    public static void unregisterTestFlow(String name) {
        ...
        if (instance != null && instance.getHandler() != null) {
            instance.getHandler().onTestFlowEnd(instance);
            Logger.info("Unregistered test flow: {}", name);
        }
    }
    ...
    private static void loadTestFlowHandler(ProxyTestFlowInstance instance) {
        try {
            ...
            if (ProxyTestFlowHandler.class.isAssignableFrom(handlerClass)) {
                ...
                Logger.info("Successfully loaded test flow handler: {}", handlerName);
            } else {
                Logger.error("Handler {} does not extend ProxyTestFlowHandler", handlerName);
            }
        } catch (Exception e) {
            Logger.error(e, "Failed to load test flow handler: {}", instance.getHandlerClassName());
        }
    }
