...
public class EnchantmentDragonHunter implements Ench, EnchFromTable {

    public static final double[] increases = new double[]{5.0, 10.0, 15.0, 20.0, 25.0, 30.0};

    @Override
    public String getDescription(int level) {
        return "Increases damage against Ender Dragons by §a" + increases[level - 1] + "%§7.";
    }
    ...
    public List<EnchantItemGroups> getGroups() {
        return List.of(EnchantItemGroups.SWORD, EnchantItemGroups.LONG_SWORD , EnchantItemGroups.FISHING_WEAPON);
    }

    @Override
    public ItemStatistics getStatistics(int level) {
        double increase = increases[level - 1];
        return ItemStatistics.builder().with(ItemStatistic.DAMAGE_MULTIPLICATIVE, increase).build();
    }
