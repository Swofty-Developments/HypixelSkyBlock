...
public enum LootAffector {
    MAGIC_FIND((player, original) -> {
        return original + (original * player.getStatistics().allStatistics().get(ItemStatistic.MAGIC_FIND) * 0.01);
    }),
    PET_LUCK((player, original) -> {
        return original + (original * player.getStatistics().allStatistics().get(ItemStatistic.PET_LUCK) * 0.01);
    }),
