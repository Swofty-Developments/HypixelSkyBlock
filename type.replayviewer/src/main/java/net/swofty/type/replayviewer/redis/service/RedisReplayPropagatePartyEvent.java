...
public class RedisReplayPropagatePartyEvent implements ServiceToClient {
    ...
    public JSONObject onMessage(JSONObject message) {
        try {
            ...
            // Special handling for warp events on replay viewer
            if (event instanceof PartyWarpResponseEvent warpEvent) {
                return handleWarpEventWithReplayValidation(warpEvent, participants);
            }
    ...
    private JSONObject handleWarpEventWithReplayValidation(PartyWarpResponseEvent warpEvent, List<UUID> participants) {
        UUID warperUUID = warpEvent.getWarper();

        HypixelPlayer warper = findPlayerByUuid(warperUUID);
        if (warper == null) {
            List<UUID> playersHandled = handleEventForPlayers(warpEvent, participants);
            return createSuccessResponse(playersHandled.size(), playersHandled);
        }

        // Warper IS on this replay viewer server - check if they're watching a replay
        ReplaySession warperSession = TypeReplayViewerLoader.getSession(warperUUID).orElse(null);

        if (warperSession == null) {
            // Warper is on this server but not in a replay - handle normally
            List<UUID> playersHandled = handleEventForPlayers(warpEvent, participants);
            return createSuccessResponse(playersHandled.size(), playersHandled);
        }

        // Warper is in a replay session - allow party members to join
        // No slot limits for replay viewing (unlimited spectators)
        List<UUID> membersToWarp = participants.stream()
                .filter(uuid -> !uuid.equals(warperUUID))
                .toList();

        List<UUID> accepted = new ArrayList<>(membersToWarp);

        // Notify the warper
        warper.sendMessage(Component.text("Warping party to replay...", net.kyori.adventure.text.format.NamedTextColor.GRAY));

        // Transfer accepted players
        for (UUID uuid : accepted) {
            // Set the replay ID so they join the same session
            RedisChosenMap.replay.put(uuid, warperSession.getReplayId().toString());
            ...
            if (memberProxy.isOnline().join()) {
                ...
                if (memberServer != null && !memberServer.uuid().equals(HypixelConst.getServerUUID())) {
                    ...
                    memberProxy.sendMessage("§eParty Leader summoned you to their replay!");
                    memberProxy.transferToWithIndication(HypixelConst.getServerUUID());
                }
            }
        }

        return createSuccessResponse(accepted.size(), accepted);
    }
    ...
    private HypixelPlayer findPlayerByUuid(UUID uuid) {
    ...
    private void handlePlayerSwitchedServerEvent(HypixelPlayer player, PartyPlayerSwitchedServerResponseEvent event) {
        if (!event.getMover().equals(player.getUuid())) {
            ...
            String displayName = "§eReplay Travel";
    ...
    private void handleTransferEvent(HypixelPlayer player, PartyLeaderTransferResponseEvent event) {
        if (event.getNewLeader().equals(player.getUuid())) {
            ...
            UUID oldLeader = event.getOldLeader();

            String newLeaderName = HypixelPlayer.getDisplayName(newLeader);
            String oldLeaderName = HypixelPlayer.getDisplayName(oldLeader);

            sendMessage(player, oldLeaderName + " §ehas transferred the party leadership to " + newLeaderName + "§e!");
        }
    }
    ...
    private void handleLeaveEvent(HypixelPlayer player, PartyMemberLeaveResponseEvent event) {
        if (!event.getLeaver().equals(player.getUuid())) {
            ...
            String leaverName = HypixelPlayer.getDisplayName(leaver);

            sendMessage(player, leaverName + " §ehas left the party.");
        }
    }
    ...
    private void handleWarpEvent(HypixelPlayer player, PartyWarpResponseEvent event) {
        if (!event.getWarper().equals(player.getUuid())) {
            ...
            if (warperServer.uuid().equals(HypixelConst.getServerUUID())) {
                // Warper is on this server - check if they have an active replay session
                ReplaySession warperSession = TypeReplayViewerLoader.getSession(warper).orElse(null);
                if (warperSession != null) {
                    // Set the replay ID so they join the same session
                    RedisChosenMap.replay.put(player.getUuid(), warperSession.getReplayId().toString());
                }
                return;
            }
