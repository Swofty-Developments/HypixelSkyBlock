...
public class GUIWithdraw extends SkyBlockInventoryGUI implements RefreshingGUI {

    public GUIWithdraw() { super("Bank Withdrawal", InventoryType.CHEST_4_ROW); }
    ...
    public void refreshItems(SkyBlockPlayer player) {
        ...
        set(new GUIClickableItem() {
            ...
            public void run(InventoryPreClickEvent e, SkyBlockPlayer player) {
                addCoins(coins , bankCoins.getValue());
                subtractCoins(bankCoins , bankCoins.getValue());
            ...
            public ItemStack.Builder getItem(SkyBlockPlayer player) {
                return ItemStackCreator.getStack(
                        "§aEverything in the account", Material.DROPPER, (short) 0, 64,
                        "§8Bank withdrawal",
                        " ",
                        "§7Current balance: §6" + StringUtility.commaify(bankCoins.getValue()),
                        "§7Amount to withdraw: §6" + StringUtility.commaify(bankCoins.getValue()),
                        " ",
        ...
        set(new GUIClickableItem() {
            @Override
            public void run(InventoryPreClickEvent e, SkyBlockPlayer player) {
                double withdraw = bankCoins.getValue() / 2;
                addCoins(coins , withdraw);
                subtractCoins(bankCoins , withdraw);
            ...
            public ItemStack.Builder getItem(SkyBlockPlayer player) {
                return ItemStackCreator.getStack(
                        "§aHalf the account", Material.CHEST, (short) 0, 32,
                        "§8Bank withdrawal",
                        " ",
                        "§7Current balance: §6" + StringUtility.commaify(bankCoins.getValue()),
                        "§7Amount to withdraw: §6" + StringUtility.commaify(bankCoins.getValue() / 2),
                        " ",
                        "§eClick to withdraw coins!"
                );
            }
        ...
        set(new GUIQueryItem() {
            ...
            public SkyBlockInventoryGUI onQueryFinish(String query, SkyBlockPlayer player) {
                double withdraw = 0;
                try{
                    withdraw = Double.parseDouble(query);
                }catch (NumberFormatException exception){
                    player.sendMessage("invalid number!");
                    return null;
                }
                if (withdraw > bankCoins.getValue()){
                    player.sendMessage("you don't have enough coins in your account!");
                    return null;
                }
                addCoins(coins , withdraw);
                subtractCoins(bankCoins , withdraw);
