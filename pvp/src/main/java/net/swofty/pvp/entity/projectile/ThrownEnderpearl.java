...
public class ThrownEnderpearl extends CustomEntityProjectile implements ItemHoldingProjectile {
	...
	private final PacketProvider particleSender;

	public ThrownEnderpearl(@Nullable Entity shooter, FallFeature fallFeature, PacketProvider packetProvider) {
		super(shooter, EntityType.ENDER_PEARL);
		this.fallFeature = fallFeature;
		this.particleSender = packetProvider;
	}

	public ThrownEnderpearl(@Nullable Entity shooter, FallFeature fallFeature) {
		super(shooter, EntityType.ENDER_PEARL);
		this.fallFeature = fallFeature;
		this.particleSender = PacketProvider.DEFAULT;
	}
	...
	private void teleportOwner() {
		...
		for (int i = 0; i < 32; i++) {
			particleSender.sendPacket(this, new ParticlePacket(
